/* Taxonomy document parsing and validation — pure, no I/O.
 *
 * The gate on an IMPORT path: a client's whole tag tree is replaced from a JSON file, and tags are
 * what every filter, Obsidian export and asset row references BY KEY. A malformed document that
 * slips through does not fail loudly — it quietly reshapes a client's taxonomy.
 *
 * Hence the hierarchy checks are as strict as the field checks: a dangling parent, a self-parent or
 * a cycle would each build a broken tree, and a cycle would build an infinite one.
 */
