...
async function fetchStatsMap(ids) {
...
export async function fetchAssets(opts = {}) {
    ...
    if (filters.perms?.length)
        query = query.in('perm', filters.perms);
    ...
    if (allAssets.length > 0) {
        ...
        const [{ data: childRows }, { data: variantRows }] = await Promise.all([
            supabase.from('assets').select('parent_id').in('parent_id', parentIds)
                .neq('status', 'archived').neq('status', 'disconnected'),
            supabase.from('assets').select('variant_of').in('variant_of', parentIds)
                .neq('status', 'archived').neq('status', 'disconnected'),
        ]);
...
export async function fetchAsset(id) {
    ...
    return toAsset({ ...data, stats: statsMap.get(id) ?? null });
}
export async function fetchChildAssets(parentId) {
    ...
    if (error)
        throw new Error(error.message);
    return data.map(row => toAsset({ ...row, stats: null }));
}
/** Folder-based stable identity variants (Task 3) — siblings of a primary asset row. */
export async function fetchVariants(primaryId) {
    if (!supabase)
        throw new Error('Supabase not configured');
    const { data, error } = await supabase
        .from('assets')
...
export async function updateAssetPerm(id, perm) {
    if (!supabase)
        throw new Error('Supabase not configured');
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    const { error } = await supabase.from('assets').update({ perm }).eq('id', id);
    if (error)
        throw new Error(error.message);
}
...
export async function deleteDisconnectedAssets(clientId) {
    if (!supabase)
        throw new Error('Supabase not configured');
    const { data, error } = await supabase
        .from('assets')
    ...
    const blocked = [];
    for (const row of data ?? []) {
        const { error: delError } = await supabase.from('assets').delete().eq('id', row.id);
        if (delError)
