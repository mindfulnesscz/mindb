...
function buildGroups(rows, dim) {
    ...
    if (orphans.length > 0)
        groups.push({ id: '', name: '', items: orphans });
    return groups;
}
export function useTags(clientId) {
    const empty = {
        entity: [], format: [], angle: [],
    ...
    useEffect(() => {
        if (!clientId || !supabase || !isConfigured())
            return;
        supabase
            .from('tags')
