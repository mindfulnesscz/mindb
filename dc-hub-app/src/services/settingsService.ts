...
function mapRawToSettings(raw: Record<string, unknown>): Partial<AppSettings> {
  ...
  if (str(raw.dropbox_app_key))      m.dropboxAppKey      = str(raw.dropbox_app_key)!;
  if (str(raw.dropbox_token))        m.dropboxToken       = str(raw.dropbox_token)!;
  if (str(raw.dropbox_refresh_token))m.dropboxRefreshToken = str(raw.dropbox_refresh_token)!;
  if (str(raw.onedrive_client_id))   m.onedriveClientId   = str(raw.onedrive_client_id)!;
  if (str(raw.onedrive_tenant_id))   m.onedriveTenantId   = str(raw.onedrive_tenant_id)!;
  if (str(raw.onedrive_token))       m.onedriveToken      = str(raw.onedrive_token)!;
  if (str(raw.onedrive_refresh_token)) m.onedriveRefreshToken = str(raw.onedrive_refresh_token)!;
