 ...
 *
 * Requires a running full local stack (`npm run db:start`). A missing Kong/edge runtime is a failure:
 * a green result must mean that every checked-out function was actually loaded.
 */
...
/* Statuses that prove the module booted: it ran far enough to make a decision.
   401 is valid only because every probe carries a required, non-empty anon key. That gets past the
   API gateway so the 401 comes from the function's own auth check; without it, Kong would reject the
   request before the function loaded and a syntax error would still look green. */
const BOOTED = new Set([400, 401, 403, 405, 422, 503]);
...
function localStack() {
  const res = spawnSync('npx', ['supabase', 'status', '-o', 'json'], {
    cwd: ROOT,
    encoding: 'utf8',
    timeout: 15_000,
  });
  ...
  try {
    ...
    return json.API_URL && json.ANON_KEY ? { url: json.API_URL, anonKey: json.ANON_KEY } : null;
  } catch { return null; }
}

export function classifyProbe(name, status, body) {
  const flat = body.replace(/\s+/g, ' ');

  // Checked BEFORE the status set, because a boot failure is a 503 and so is "not provisioned".
  if (BOOT_FAILURE.test(body)) return { name, ok: false, detail: `${status} — DID NOT BOOT: ${flat}` };
  if (BOOTED.has(status)) return { name, ok: true, detail: `${status} (booted)` };

  return { name, ok: false, detail: `${status} — unexpected: ${flat}` };
}

export function requireStack(stack) {
  if (!stack) {
    throw new Error('no full local Supabase stack. Start one with `npm run db:start`.');
  }
  return stack;
}
...
async function probe(base, anonKey, name) {
  ...
  return classifyProbe(name, res.status, body);
}
...
async function main() {
  ...
  if (process.argv.includes('--fresh')) {
    ...
    if (started.status !== 0) {
      throw new Error('could not restart the local stack.');
    }
  }

  const stack = requireStack(localStack());
...
if (resolve(process.argv[1] ?? '') === fileURLToPath(import.meta.url)) {
  main().catch(err => {
    console.error(`smoke-functions: ${err.message}`);
    process.exit(1);
  });
}
