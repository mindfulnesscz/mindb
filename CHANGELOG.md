...
- R2/CDN bucket CORS policy set in the Cloudflare dashboard (`GET`/`HEAD` from `*`), enabling the blob-download path — the browser's save dialog now works for cross-origin CDN files
