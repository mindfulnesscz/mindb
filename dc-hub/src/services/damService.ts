...
import type { RunContext } from './pipelineService';
import {
...
export async function runObsidian(ctx: RunContext, stats: RunStats): Promise<void> {
  ...
  if (!outDirs.length) {
    appendLog('dim', `  No ${settings.outFolder} folders found.`);
  ...
  for (let idx = 0; idx < outDirs.length; idx++) {
    ...
    for (const gName of galleryNames) {
      ...
      if (await fileExists(notePath)) {
        try {
          ...
          const { content: patched, changed } = patchMeta(existing, gParsed, projRel, thumbName);
          if (changed) {
        ...
        try {
          await writeTextFile(notePath, makeNote(gParsed, gName, projRel, exportName || null, thumbName));
    ...
    for (const file of assetFiles) {
      ...
      if (await fileExists(notePath)) {
        try {
          const existing = await readTextFile(notePath);
          const { content: patched, changed } = patchMeta(existing, parsed, projRel, thumbName);
          if (changed) {
            await writeTextFile(notePath, patched);
            appendLog('success', `    ↑  updated: ${noteFileName}`);
            stats.notes += 1;
        ...
        try {
          await writeTextFile(notePath, makeNote(parsed, file.name, projRel, exportName, thumbName));
